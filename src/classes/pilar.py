...
class Pilar:
    ...
    def rolling_min_variance_optimization(self, calculation_window: int, 
        ...
        # Maintenant, propager les poids à toutes les dates dans l'index des rendements en tenant compte de la dérive
        # Initialisation du DataFrame pour toutes les dates
        all_weights = pd.DataFrame(index=self.returns.index, columns=assets)
        
        # Pour chaque date de rebalancement, propager les poids jusqu'au prochain rebalancement
        # en tenant compte de la dérive des actifs
        ...
        for i in range(len(sorted_rebal_dates) - 1):
            ...
            # Sélectionner les dates entre deux rebalancement (inclusif pour start_date, exclusif pour end_date)
            mask = (self.returns.index >= start_date) & (self.returns.index < end_date)
            dates_between = self.returns.index[mask]
            
            # Poids au début de la période de rebalancement
            current_weights = rebal_weights.loc[start_date].values
            
            # Assigner les poids initiaux à la date de rebalancement
            all_weights.loc[start_date] = current_weights
            
            # Pour chaque date entre les rebalancement, calculer la dérive
            prev_date = start_date
            for current_date in dates_between[1:]:  # Saute la première date qui est déjà assignée
                # Récupérer les rendements pour la période entre prev_date et current_date
                if prev_date in self.returns.index and current_date in self.returns.index:
                    ...
                    if current_idx > prev_idx:
                        # Récupérer les rendements pour cette période
                        period_returns = self.returns.iloc[prev_idx]
                        
                        # Calculer les nouveaux poids après dérive
                        # Etape 1: calculer le rendement du portefeuille
                        portfolio_return = np.sum(current_weights * period_returns)
                        total_return = 1 + portfolio_return
                        
                        # Etape 2: calculer les nouveaux poids après dérive
                        new_weights = []
                        ...
                        # Mettre à jour les poids courants pour la prochaine itération
                        current_weights = np.array(new_weights)
                        
                        # Assigner les nouveaux poids à la date courante
                        all_weights.loc[current_date] = current_weights
                
                # Mettre à jour la date précédente pour la prochaine itération
                prev_date = current_date
        
        # Traitement après le dernier rebalancement : calcul de la dérive
        if len(sorted_rebal_dates) > 0:
            ...
            # Pour chaque date après le dernier rebalancement, on calcule la dérive
